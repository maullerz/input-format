...
function onPaste(event, input, _parse, _format, on_change) {
  ...
  const selection = (0, _dom.getSelection)(input); // If selection is made,
...
function onKeyDown(event, input, _parse, _format, on_change) {
  ...
  const operation = (0, _dom.getOperation)(event);
  ...
  switch (operation) {
    ...
    case 'Backspace':
      ...
      const selection = (0, _dom.getSelection)(input); // If a selection is made, just erase the selected text.
...
function formatInputText(input, _parse, _format, operation, on_change) {
  ...
  let {
    value,
    caret
  } = (0, _parse2.default)(input.value, (0, _dom.getCaretPosition)(input), _parse); // If a user performed an operation ("Backspace", "Delete")
  // then apply that operation and get the new `value` and `caret` position.

  if (operation) {
    const newValueAndCaret = (0, _edit.default)(value, caret, operation);
  ...
  const formatted = (0, _format2.default)(value, caret, _format);
