...
export function onPaste(event, input, _parse, _format, on_change) {
  ...
  const selection = getSelection(input); // If selection is made,
...
export function onKeyDown(event, input, _parse, _format, on_change) {
  ...
  switch (operation) {
    ...
    case 'Backspace':
      ...
      const selection = getSelection(input); // If a selection is made, just erase the selected text.
...
function formatInputText(input, _parse, _format, operation, on_change) {
  ...
  let {
    value,
    caret
  } = parse(input.value, getCaretPosition(input), _parse); // If a user performed an operation ("Backspace", "Delete")
